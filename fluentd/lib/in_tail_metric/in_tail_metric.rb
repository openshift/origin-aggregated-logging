  ...
  class TailInput < Fluent::Plugin::Input
    ...
    class TailWatcher
      def initialize(path, rotate_wait, pe, log, read_from_head, enable_watch_timer, enable_stat_watcher, read_lines_limit, write_watcher, update_watcher, line_buffer_timer_flusher, from_encoding, encoding, open_on_every_update, &receive_lines)
        ...
        @totalbytes_logged_in_disk=0
        @totalbytes_collected_from_disk=0
      end
      ...
      attr_accessor :totalbytes_logged_in_disk
      attr_accessor :totalbytes_collected_from_disk
      ...
      def on_notify
        ...
        @totalbytes_logged_in_disk=@write_watcher.update_totalbytes_logged_in_disk(@path)
        @line_buffer_timer_flusher.on_notify(self) if @line_buffer_timer_flusher
        @io_handler.on_notify if @io_handler
        @totalbytes_collected_from_disk=@write_watcher.update_totalbytes_logged_in_disk(@path)
      end
              ...
              if !io.nil? && @lines.empty?
                begin
                  while true
                    ...
                    @write_watcher.count_totalbytes_collected_from_disk(@bytesread,@watcher.path)
                    @fifo << buf
