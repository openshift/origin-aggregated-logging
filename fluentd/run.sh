...
# Copy the Kafka configuration into the OpenShift directory
cp $CFG_DIR/output-kafka.conf $CFG_DIR/openshift
if [ -f "$CFG_DIR/user/output-kafka.conf" ]; then
    cp -f $CFG_DIR/user/output-kafka.conf $CFG_DIR/openshift
fi
