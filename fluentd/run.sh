...
# Copy the multiline aggregation configuration to the openshift directory,
# including any user-defined files
cp $CFG_DIR/filter-*-multiline.conf $CFG_DIR/openshift
for file in $CFG_DIR/user/filter-*-multiline.conf; do
    if [ -f "$file" ]; then
        cp -f $file $CFG_DIR/openshift
    fi
done
