...
func main() {
	...
	queryCommand := `oc exec ` + kibana_pod + ` -c kibana -- curl -s -k --key /etc/kibana/keys/key --cert /etc/kibana/keys/cert -XGET "https://` + es_svc + `/` + index + `.*/fluentd/_search?q=hostname:` + hostname + `&fields=message&size=` + querySize + `"`
	if verbose {
